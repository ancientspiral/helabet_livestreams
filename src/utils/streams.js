...
export const deriveScheduleBucket = (
  isoString,
  now = new Date(),
  timeZone = DEFAULT_REGION_TZ,
) => {
  ...
  const diffDays = Math.round(
    (targetMidnight.getTime() - currentMidnight.getTime()) / MS_PER_DAY,
  );
  ...
  if (
    targetWeek.year === currentWeek.year &&
  ...
  const monthDiff =
    (targetYMD.year - currentYMD.year) * 12 +
