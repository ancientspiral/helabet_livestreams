...
export interface MarketingEnvConfig {
  baseUrl: string;
  authUrl: string;
  clientId?: string;
  clientSecret?: string;
...
export const loadMarketingEnvConfig = (
  env: NodeJS.ProcessEnv = process.env,
): MarketingEnvConfig => {
  const periods = parseCommaList(env.MARKETING_API_PERIODS);
  const cacheTtlMs = Math.max(
    1_000,
    parseNumber(env.MARKETING_API_CACHE_TTL_MS, DEFAULT_CACHE_TTL_MS),
  );
  ...
  const upcomingDays = Math.max(
    1,
    parseNumber(env.MARKETING_API_UPCOMING_DAYS, DEFAULT_UPCOMING_DAYS),
  );
  const liveLookbackMinutes = Math.max(
    0,
    parseNumber(
      env.MARKETING_API_LIVE_LOOKBACK_MINUTES,
      DEFAULT_LIVE_LOOKBACK_MINUTES,
    ),
  );
  const prematchLookaheadMinutes = Math.max(
    1,
    parseNumber(
      env.MARKETING_API_PREMATCH_LOOKAHEAD_MINUTES,
      DEFAULT_PREMATCH_LOOKAHEAD_MINUTES,
    ),
  );
