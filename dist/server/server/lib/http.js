...
export const fetchWithRetry = async (url, init, maxRetries = 3) => {
    let attempt = 0;
    let lastError;
    ...
    while (attempt < maxRetries) {
        try {
            ...
            const shouldRetry = RETRYABLE_STATUS.has(response.status) || response.status >= 500;
            if (!shouldRetry) {
                return response;
            }
        }
...
export const dumpCacheKeys = () => Array.from(cacheStore.keys());
