...
import { HelabetProxyError, apiGetChampZip, apiGetChampsZip, apiGetCinemaUrl, } from "../lib/helabet.js";
...
const parseBooleanQuery = (value) => value === "1" || value === "true";
const respondWithError = (res, status, error) => {
    res.status(status).json({ error });
};
const handleError = (error, res, fallback) => {
    if (error instanceof HelabetProxyError) {
        respondWithError(res, error.status, error.message);
        return;
    }
    const statusCandidate = typeof error?.status === "number"
        ? Number(error.status)
        : null;
    const message = error instanceof Error && error.message ? error.message : fallback;
    respondWithError(res, statusCandidate && statusCandidate >= 400 ? statusCandidate : 502, message);
};
