...
const makeCacheKey = (videoId, sgi) => `${videoId}::${sgi ?? ""}`;
export const deriveTtlFromUrl = (rawUrl) => {
    if (!rawUrl) {
        return null;
    }
    try {
...
const buildResolvePayload = (url, ttlSource) => {
...
const logResolve = (stage, method, url, status, snippet) => {
...
const decodeHtml = (value) => value
    ...
    .replace(/&lt;/g, "<")
    .replace(/&gt;/g, ">");
const findM3u8Urls = (value, results, seen) => {
    ...
    if (typeof value === "object") {
        ...
        Object.values(value).forEach((entry) => findM3u8Urls(entry, results, seen));
    }
};
const pickM3u8 = (urls) => {
    if (!urls.length) {
...
const findFirstNumber = (value, key, seen) => {
...
const findFirstString = (value, key, seen) => {
    if (!value || typeof value !== "object") {
        return null;
    }
    if (seen.has(value)) {
...
const extractJsonBlobs = (html) => {
    const blobs = [];
    const nextDataRegex = /<script id="__NEXT_DATA__" type="application\/json">([^<]+)<\/script>/g;
    let match;
    ...
    const windowRegex = /window\.(?:__NUXT__|INITIAL_STATE)\s*=\s*(\{[\s\S]*?\});/g;
...
const attemptBffResolve = async (session, videoId) => {
...
const attemptCinemaResolve = async (session, videoId) => {
    const path = "/cinema";
    const body = {
        AppId: 3,
        AppVer: "1025",
...
const attemptHtmlResolve = async (session, videoId, sgi) => {
    const paths = [];
    if (videoId) {
    ...
    for (const path of paths) {
        try {
            ...
            const urls = [];
            payloads.forEach((payload) => findM3u8Urls(payload, urls, new WeakSet()));
            if (!urls.length) {
                const fallbackMatches = text.match(/https?:[^"'<>\s]+\.m3u8[^"'<>\s]*/gi) ?? [];
...
const executeResolve = async (session, rawVideoId, rawSgi, cacheKey) => {
    const videoId = typeof rawVideoId === "string" ? rawVideoId.trim() : "";
    const sgi = typeof rawSgi === "string" && rawSgi.trim() ? rawSgi.trim() : undefined;
    ...
    const httpFailures = attempts.filter((attempt) => attempt.kind === "http");
    if (httpFailures.length > 0 &&
        httpFailures.every((failure) => failure.status >= 400 && failure.status < 500)) {
...
const getInflight = (session, cacheKey, videoId, sgi) => {
...
const sendError = (res, status, options = {}) => {
    if (status === 404) {
 ...
 */
export function makeResolveHandler(session) {
    return async function resolveHandler(req, res) {
        res.type("application/json");
        const rawVideoId = req.body?.videoId;
        const videoId = typeof rawVideoId === "string" ? rawVideoId.trim() : "";
        const rawSgi = req.body?.sgi;
        const sgi = typeof rawSgi === "string" && rawSgi.trim() ? rawSgi.trim() : undefined;
        if (!videoId && !sgi) {
        ...
        try {
            ...
            console.warn("[resolve][error]", error?.message ?? "resolve_failed");
