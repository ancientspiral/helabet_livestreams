...
const normalizeId = (value) => {
    ...
    if (typeof value === "string") {
        ...
        if (Number.isFinite(numeric) && String(numeric) === trimmed.replace(/^0+/, "") && numeric > 0) {
...
export async function fetchHelabetVideoLookup(session) {
    try {
        const response = await session.helabetRequest(HELABET_VIDEO_ENDPOINT, { method: "GET" });
        if (!response.ok) {
            console.warn("[helabet-video][warn]", "live feed request failed", response.status, response.statusText);
            return new Map();
        }
        ...
        console.warn("[helabet-video][error]", error?.message ?? String(error));
        return new Map();
    }
}
