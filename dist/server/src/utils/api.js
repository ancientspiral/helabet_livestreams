import { generateStreamId, deriveScheduleBucket, DEFAULT_TIME_ZONE, } from "./streams.js";
...
export const normalizeOdds = (odds) => {
    if (!odds)
        return null;
    const hasNewStructure = typeof odds.w1 === "number" ||
    ...
    if (hasNewStructure) {
        ...
        if (typeof odds.x === "number" && !Number.isNaN(odds.x))
            normalized.x = odds.x;
        if (typeof normalized.w1 !== "number" ||
...
export const extractOdds1x2 = (entries = []) => {
    ...
    const filtered = entries.filter((entry) => {
        if (!entry || typeof entry !== "object")
            return false;
        if (entry.G !== undefined && entry.G !== 1)
            return false;
        if (entry.GS !== undefined && entry.GS !== 1)
            return false;
        return true;
    });
    for (const entry of filtered) {
        if (!entry || typeof entry !== "object")
            continue;
        const value = parseOddComponent(entry.C ?? entry.V ?? entry.value ?? entry.Odd);
        if (value === null) {
            continue;
        }
        if (entry?.T === 1) {
...
export const mapMatchToStream = (match) => {
    ...
    const fallbackVideoId = typeof match.videoId === "string" && match.videoId.trim()
        ? match.videoId.trim()
        : null;
    const vi = typeof viRaw === "string" && viRaw.trim() ? viRaw.trim() : null;
    const sgi = typeof sgiRaw === "string" && sgiRaw.trim() ? sgiRaw.trim() : null;
    ...
    const videoSource = vi
        ...
        : fallbackVideoId
            ...
            : sgi
                ? "sgi"
                : "other";
    const matchId = match.CI ?? match.Id ?? match.matchId ?? match.id ?? match.VI ?? null;
    const sportId = match.SI ?? match.sportId ?? match.sport ?? null;
    const sportNameRaw = match.SE ??
        match.SN ??
        match.sportName ??
        match.sportTitle ??
        match.sportLabel ??
    ...
    const baseId = typeof matchId === "number" && Number.isFinite(matchId)
        ? `helabet-${matchId}`
        : typeof matchId === "string" && matchId
            ? `helabet-${matchId}`
            : videoId ?? sgi ?? undefined;
    const hasOdds = rawOdds &&
    ...
    const odds = hasOdds
        ...
        : null;
    const startTimeMs = typeof startMs === "number"
        ? startMs
        : dateISO
