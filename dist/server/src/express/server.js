...
app.get("/api/live/one-x-two", (_req, res) => {
    ...
    res.redirect(307, "/api/hlb/service-api/LiveFeed/Get1x2_VZip?lid=1&lng=en&antisports=66&partner=237");
});
...
app.use((error, _req, res, _next) => {
    console.error("[express]", error?.message ?? String(error));
...
void session
    ...
    .catch((error) => {
    console.warn("[helabet] warm-up failed", error?.message ?? String(error));
});
