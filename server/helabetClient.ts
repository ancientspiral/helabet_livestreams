...
let X_HD: string | undefined; // current token from video.json response header
let lastWarmup = 0;
...
const getCookieHeader = (origin: string): string | undefined => {
...
const storeCookies = (
  ...
  const jar = cookieStore.get(origin) ?? new Map<string, string>();
...
const applyCookies = (headers: HeaderRecord, origin: string): HeaderRecord => {
...
const requestWithHeaders = async (
  ...
  init?: Omit<Parameters<typeof request>[1], "headers">,
...
async function getJson(url: string): Promise<unknown> {
  ...
  if (res.statusCode === 406) {
    const err = new Error("406") as Error & { code?: number };
    err.code = 406;
    throw err;
  }
...
async function getWithWarmup(url: string): Promise<unknown> {
  await warmUp();
  try {
...
export async function apiGetChampZip(champId: number | string) {
...
export async function apiCinema(videoId: string) {
